...
mint f[N][N],g[N][N],suf[N];
void solve() {
    cin>>n>>a>>b>>p>>q;
    f[a][0]=1;g[b][0]=1;
    mint invp=mint(p).inv(),invq=mint(q).inv();
    // cout<<invp<<"\n";
    for(int i=a; i<n; i++) {
    	for(int j=0; j<=n; j++) {
    		for(int k=1; k<=p; k++) {
    			f[min(i+k,n)][j+1]+=f[i][j]*invp;
            }
    ...
    for(int i=b; i<n; i++) {
    	for(int j=0; j<=n; j++) {
        		for(int k=1; k<=q; k++) {
    			g[min(i+k,n)][j+1]+=g[i][j]*invq;
    		}
    	}
    }
    // for(int i=1; i<=n; i++) cout<<f[n][i]<<" \n"[i==n];
    // for(int i=1; i<=n; i++) cout<<g[n][i]<<" \n"[i==n];
    for(int i=n; i>=1; i--) suf[i]=suf[i+1]+g[n][i];
    mint ans=0;
    for(int i=1; i<=n; i++) {
        ans+=f[n][i]*suf[i];
    }
