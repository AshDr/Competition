...
int n;
char s[N];
mint dp[N],pw[N],sum[N];
int a[N];
void solve() {
	cin >> n;
	cin >> (s+1);
    pw[0]=1;
    for(int i = 1; i <= n; i++) a[i]=s[i]-'0',pw[i]=pw[i-1]*10;
    //xyzp
    //dp[2]=(dp[0]*xy+dp[1]*y);
    //dp[3]=(dp[0]*xyz+dp[1]*yz+dp[2]*z)
    //dp[4]=(dp[0]*xyz p+dp[1]*yz p+dp[2]*z p+dp[3]*p)
    dp[0]=1;dp[1]=a[1];
    //dp[1]=2 sum[1]=(dp[0]+dp[1]);
    //23+6
    sum[0]=1;
    sum[1]=dp[1]+dp[0];
    for(int i = 2; i <= n; i++) {
        //2*(10+3)
        dp[i]=dp[i-1]*10+sum[i-2]*a[i]+dp[i-1]*a[i];
        sum[i]=sum[i-1]+dp[i];
        // cout << i << " "<<dp[i] << "\n";
    }
    cout<<dp[n]<<"\n";
