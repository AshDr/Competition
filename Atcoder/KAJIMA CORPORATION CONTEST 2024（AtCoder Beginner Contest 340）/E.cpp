...
void solve() {
    ...
    for(int i = 0; i < m; i++) {
    	int pos = b[i];
    	ll num = seg.rangeQuery(pos, pos + 1).sum;
    	seg.modify(pos, {0});
    	ll t = num / n, rem = num % n;
    	seg.rangeApply(0, n, {t});
    	if(pos + rem < n) {
    		seg.rangeApply(pos + 1, pos + rem + 1, {1});
    	}else {
    		seg.rangeApply(pos + 1, n, {1});
    		seg.rangeApply(0, (pos + rem) % n + 1, {1});
    	}	
    }
    for(int i = 0; i < n; i++) {
    	cout << seg.rangeQuery(i, i + 1).sum << " ";
    }
}
int main() {
    #ifdef ASHDR
