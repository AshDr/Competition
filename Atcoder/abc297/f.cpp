	    	...
	    	for(int j=1; j<=w; j++) {
	    		if(i*j<k) continue;
                    ...
                    for(int mask=0; mask<(1<<4); mask++) {
                        int pari = __builtin_parity(mask);
                        int t1=__builtin_popcount((mask&3)),t2=__builtin_popcount((mask&12));
                        int sign=(pari?-1:1);
                        res+=sign*binom(max(0,i-t1)*max(0,j-t2),k);
                    }
                    //coef=res
                    ans+=res*i*j*(h-i+1)*(w-j+1);
                } 
	    	}
	    }
	    ans*=binom(h*w, k).inv();
