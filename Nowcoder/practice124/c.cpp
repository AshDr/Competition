...
void solve() {
    ...
    disc.init();
    Fenwick<int> fw(sz(disc.d) + 2);
    sort(all(a));
    ll ans = 0;
    for(int i = 1; i <= n;) {
    	int j = i;
    	while(j <= n && a[j].r == a[i].r) {
    		fw.modify(disc.query(a[j].l), 1);
    		++j;
    	}
    	if(fw.query(disc.query(a[i].r)) >= k) {
    		int l = 1, r = a[i].r;
	    	auto check = [&](int x) {
	    		return fw.query(disc.query(x), disc.query(a[i].r)) >= k;
	    	};
	    	while(l < r) {
	    		int mid = (l + r + 1) >> 1;
	    		if(check(mid)) l = mid;
	    	...
	    	// cout << l << " " << a[i].r << "\n"; 
	    	ans += ((j == n + 1 ? m + 1: a[j].r) - a[i].r) * 1ll * l;
    	}
    	i = j;
    }
    cout << ans << "\n";
