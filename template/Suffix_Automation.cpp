...
ll getstrnum() { return dfs(1); }
}  // namespace SAM
   /*
   ac自动机是多个模式串构建的接受前缀的dfa
   sam是一个接受s所有后缀的dfa 是dag，从t0出发，到达一个终止状态就是s的一个后缀
   任意从初始状态 t_0 开始的路径，如果我们将转移路径上的标号写下来，都会形成 s 的一个 子串。反之每个 s 的子串对应从 t_0
   开始的某条路径。    所有字符串 s 的非空子串都可以根据它们的 endpos 集合被分为若干 等价类，因为可能endpos(s1) =
   endpos(s2)，一类串在sam中对应一个节点    SAM 中的状态数等于所有子串的等价类的个数，再加上初始状态。    一个 后缀链接 link(v)
   连接到对应于 w 的最长后缀的另一个 endpos 等价类的状态,w为等价类中最长的那个串    任意一个 SAM 的 endpos 集合形成了一棵树
   */