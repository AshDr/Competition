...
void solve() {
	...
	//printf("%.10lf\n%.10lf %.10lf\n", sqrt(r2), o.x, o.y);
	res = min(res,2 * sqrt(r2));
}
int main() {
	scanf("%d", &n);
	for(int i = 1; i <= n; i++) scanf("%lf%lf%lf",&a[i].x,&a[i].y,&a[i].z);
	for(int i = 1; i <= n; i++) pot[i].x = a[i].x,pot[i].y = a[i].y;
	solve();
	for(int i = 1; i <= n; i++) pot[i].x = a[i].y,pot[i].y = a[i].z;
	solve();
	for(int i = 1; i <= n; i++) pot[i].x = a[i].x,pot[i].y = a[i].z;
	solve(); 
	printf("%.10lf",res);
	return 0;
}