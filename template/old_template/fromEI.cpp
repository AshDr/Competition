...
int main() {
  ...
  int n, a, b; cin >> n >> a >> b;
  int p = a * (ll)nt.inv(b) % P, q = norm(P + 1 - p);
  Poly pol = zeroes(n);
  int val = mpow(n - 1, P - 1 - n);
  for (int i = 0; i <= n; ++i) {
    pol[i] = simp.binom(n, i) * (ll)val % P;
    pol[i] = pol[i] * (ull)mpow((n - 1 + (P - p) * (ull)(i + P - 1)) % P, i) % P;
    pol[i] = pol[i] * (ull)mpow((n - 1 + (P - p) * (ull)i) % P, n - i) % P;
  }
  pol = pol.taylor(P - 1);
