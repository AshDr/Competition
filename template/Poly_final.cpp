...
struct NTT {
  ...
  NTT() : L(-1) {
    for (int i = 1; i < (1 << L2); ++i) brev[i] = brev[i >> 1] >> 1 | ((i & 1) << (L2 - 1));
  }
  ...
  void prepRoot(int l) {
    ...
    for (int i = 0; i <= L; ++i) {
      ...
      for (int j = 1; j < 1 << i; ++j) w[j] = w[j - 1] * (ll)omega % P;
    }
  }
...
struct Newton {
  void inv(const Poly &f, const Poly &nttf, Poly &g, const Poly &nttg, int t) {
    int n = 1 << t;
    Poly prod = nttf;
    for (int i = 0; i < (n << 1); ++i) prod[i] = prod[i] * (ll)nttg[i] % P;
  ...
  void sqrt(const Poly &f, Poly &g, Poly &nttg, Poly &h, int t) {
    ...
    for (int i = 0; i < (2 << t); ++i) tmp[i] = tmp[i] * (ll)nttg[i] % P;
    ntt.fft(tmp.base(), t + 1, -1);
  ...
  void exp(const Poly &f, Poly &g, Poly &nttg, Poly &h, int t) {
    ...
    for (int i = 0; i < (2 << t); ++i) df0[i] = df0[i] * (ll)tmp[i] % P;
    ntt.fft(df0.base(), t + 1, -1);
    ...
    for (int i = 0; i < (2 << t); ++i) tmp[i] = dg[i] * (ll)nttg[i] % P;
    ntt.fft(tmp.base(), t + 1, -1);
    g.redeg((2 << t) - 1);
...
struct SemiRelaxedConvolution {  // 半卷积
  ...
  void run(const vector<int> &a, vector<int> &b, const Function &relax) {
    ...
    function<void(int, int)> divideConquer = [&](int l, int r) {
      if (r - l <= BRUTE_N2_LIMIT) {
        for (int i = l; i <= r; ++i) {
          for (int j = l; j < i; ++j) b[i] = (b[i] + b[j] * (ll)a[i - j]) % P;
          relax(i);
        }
        return;
      }
...
struct Transposition {
  ...
  Poly bfMul(const Poly &a, const Poly &b) {
    ...
    Poly ret = zeroes(n - m);
    for (int i = 0; i <= n - m; ++i)
      for (int j = 0; j <= m; ++j) ret[i] = (ret[i] + a[i + j] * (ll)b[j]) % P;
    return ret;
  }
  ...
  Poly inter(const vector<int> &x, const vector<int> &y) {
    ...
    for (int i = 0; i < n * 2 - 1; ++i)
      if (ls[i]) tie(p[ls[i]], p[rs[i]]) = mul2(p[i], q[rs[i]], q[ls[i]]);
    for (int i = 0; i < n; ++i) p[pos[i]] = nt.inv(p[pos[i]][0]) * (ll)y[i] % P;
...
Poly Poly::operator*(const Poly &rhs) const {
  ...
  if (n <= 10 || m <= 10 || n + m <= BRUTE_N2_LIMIT) {
    Poly ret = zeroes(n + m);
    for (int i = 0; i <= n; ++i)
      for (int j = 0; j <= m; ++j) ret[i + j] = (ret[i + j] + a[i] * (ll)rhs[j]) % P;
    return ret;
  }
  n += m;
  int l = 0;
...
Poly Poly::der() const {
  if (deg() == 0) return 0;
  vector<int> res(deg());
  for (int i = 0; i < deg(); ++i) res[i] = a[i + 1] * (ll)(i + 1) % P;
  return res;
}
...
Poly Poly::integ() const {
  ...
  simp.check(deg() + 1);
  for (int i = 0; i <= deg(); ++i) res[i + 1] = a[i] * (ll)simp.inv[i + 1] % P;
  return res;
}

Poly Poly::quo(const Poly &rhs) const {
  if (rhs.deg() == 0) return a[0] * (ll)nt.inv(rhs[0]) % P;
  Poly g = nt.inv(rhs[0]);
  int t = 0, n;
  ...
  for (int i = 0; i < (n << 1); ++i) eps1[i] = eps1[i] * (ll)nttg[i] % P;
  ntt.fft(eps1.base(), t + 1, -1);
  ...
  for (int i = 0; i < (n << 1); ++i) h0g0[i] = h0[i] * (ll)nttg[i] % P;
  ntt.fft(h0g0.base(), t + 1, -1);
  Poly h0eps1 = zeroes((n << 1) - 1);
  for (int i = 0; i < (n << 1); ++i) h0eps1[i] = h0[i] * (ll)eps1[i] % P;
  ...
  for (int i = 0; i < (n << 1); ++i) h0eps1[i] = h0eps1[i] * (ll)nttg[i] % P;
  ntt.fft(h0eps1.base(), t + 1, -1);
...
Poly Poly::exp() const {
  ...
  for (int i = 0; i < a.size(); ++i) der[i] = der[i] * (ll)i % P;
  src.run(der, ret, [&](int i) {
    if (i == 0)
      ret[0] = 1;
    else
      ret[i] = ret[i] * (ll)simp.ginv(i) % P;
  });
  return ret;
}
...
Poly Poly::exp(int k) const {
  ...
  lead = a[lz];
  if (lz * (ll)k > deg()) return zeroes(deg());
...
Poly Poly::operator/(const Poly &rhs) const {
  int n = deg(), m = rhs.deg();
  if (n < m) return 0;
  Poly ta(vector<int>(a.rbegin(), a.rend())), tb(vector<int>(rhs.a.rbegin(), rhs.a.rend()));
  ta.redeg(n - m);
  tb.redeg(n - m);
  Poly q = ta.quo(tb);
...
pair<Poly, Poly> Poly::div(const Poly &rhs) const {
  ...
  for (int i = 0; i <= t; ++i) tmp[i] = tmp[i] * (ll)r[i] % P;
  ntt.fft(tmp.base(), lgn, -1);
