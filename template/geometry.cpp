...
struct Point {
  ...
  Point rotleft() { return Point(-y, x); }  // 逆90°
...
struct Line {
  ...
  bool pointonseg(Point p) { return sgn((p - s) ^ (e - s)) == 0 && sgn((p - s) * (p - e)) <= 0; }  // 判断点是否在线段上

  bool parallel(Line v) { return sgn((e - s) ^ (v.e - v.s)) == 0; }  // 判断两向量是否平行
  ...
  Point lineprog(Point p) { return s + (((e - s) * ((e - s) * (p - s))) / ((e - s).len2())); }  // 点在直线上的投影
...
struct circle {
  ...
  void output() { cout << p.x << ' ' << p.y << ' ' << r << endl; }
...
struct polygon {
  ...
  double minRectangleCover(polygon A) {
    ...
    for (int i = 0; i < A.n; i++) {
      ...
      double tmp =
          cross(A.p[i], A.p[i + 1], A.p[r]) * (dot(A.p[i], A.p[i + 1], A.p[p]) - dot(A.p[i], A.p[i + 1], A.p[q])) / d;
