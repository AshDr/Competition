// also can use <tr2/dynamic_bitset>
struct custom_bitset {
  ...
  void _clean() {
    ...
    if (b != 64 * n) bits.back() &= (1LLU << (b - 64 * (n - 1))) - 1;
  }

  bool get(int64_t index) const { return bits[index / 64] >> (index % 64) & 1; }
  ...
  void or_shift(int64_t shift) {
    ...
    if (mod == 0) {
      for (int64_t i = n - 1; i >= div; i--) bits[i] |= bits[i - div];
  ...
  void or_shift_down(int64_t shift) {
    ...
    if (mod == 0) {
      for (int64_t i = div; i < n; i++) bits[i - div] |= bits[i];

      return;
    }

    for (int64_t i = 0; i < n - (div + 1); i++) bits[i] |= bits[i + (div + 1)] << (64 - mod) | bits[i + div] >> mod;

    if (div < n) bits[n - div - 1] |= bits[n - 1] >> mod;
  ...
  int64_t find_first() const {
    for (int i = 0; i < n; i++)
      if (bits[i] != 0) return 64 * i + __builtin_ctzll(bits[i]);
