...
void solve() {
	...
	double ans = min(max(dis(a,p),dis(a,{0ll,0ll})),max(dis(b,p),dis(b, {0ll,0ll})));
	double d = dis(a, b) / 2;
	double mn1 = min(dis(a,{0ll,0ll}),dis(b,{0ll,0ll})),mn2 = min(dis(a,p),dis(b,p));
	ans = min(ans,max({d,mn1,mn2}));
	cout << ans << "\n";
}
int main() {
    #ifdef ASHDR
